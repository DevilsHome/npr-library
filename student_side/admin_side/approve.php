...
<?php
  ...
  {
    mysqli_query($con,"UPDATE  `issue_book` SET  `approve` =  '$_POST[approve]', `issue` =  '$_POST[issue]', `return` =  '$_POST[return]' WHERE username='$_SESSION[username]' and issn_no='$_SESSION[issn_no]';");

    mysqli_query($con,"UPDATE book SET quantity = quantity-1 where issn_no='$_SESSION[issn_no]' ;");

    $res=mysqli_query($con,"SELECT quantity from book where issn_no='$_SESSION[issn_no];");

    while($row=mysqli_fetch_assoc($res))
    {
      if($row['quantity']==0)
      {
        mysqli_query($con,"UPDATE book SET status='not-available' where issn_no='$_SESSION[issn_no]';");
      }
    }
    ?>
      <script type="text/javascript">
        alert("Updated successfully.");
         window.location="requestt.php";
      </script>
    <?php
  }
?>

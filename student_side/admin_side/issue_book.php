...
<div id="main">
  ...
  <div class="container">
    ...
    $c=0;
      ...
      if(isset($_SESSION['login_user']))
      {
        $sql="SELECT student.username,firstname,book.issn_no,name,authors,edition,issue,issue_book.return FROM student inner join issue_book ON student.username=issue_book.username inner join book ON issue_book.issn_no=book.issn_no WHERE issue_book.approve !='' ORDER BY `issue_book`.`return` ASC";
      ...
      while($row=mysqli_fetch_assoc($res))
      {
        ...
        if($d > $row['return'])
        {
          ...
          mysqli_query($con,"UPDATE issue_book SET approve='$var' where `return`='$row[return]' and approve='Yes' limit $c;");
